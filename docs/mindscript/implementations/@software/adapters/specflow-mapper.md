...
This adapter connects MindScript `@software` criteria to **SpecFlow** by **referencing external artifacts only**. It does **not** embed or generate Gherkin or C# step code from MindScript. Instead, it verifies links to SpecFlow `.feature` files and `Scenario`s you reference, and produces a traceability index.

## Design principles

- **MindScript stays agnostic.** We only link to SpecFlow; no Gherkin or C# semantics live in the MindScript schema.
...
- `criteria[].bdd_ref.source` → SpecFlow profile URL  
  Example: `https://mindscript.dev/profiles/bdd/specflow`
...
## Minimal example (MindScript → SpecFlow reference)
  ...
  - id: SPECFLOW-201.1
    ...
    criteria:
      - id: SPECFLOW-201.1.1
        ...
        bdd_ref:
          source: https://mindscript.dev/profiles/bdd/specflow
...
- Tag scenarios with MindScript IDs for back-references (as shown above), or use C# category attributes on test runners if applicable
